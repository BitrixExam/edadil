...
class PerekrestokCatalogParser implements CatalogParser
{
    ...
    public function supports(string $url): bool
    {
        ...
        return str_contains($host, "perekrestok.ru")
            && (str_starts_with($path, "/cat/") || str_starts_with($path, "/cat"));
    }
    ...
    {
        return "https://www.perekrestok.ru";
    }
    ...
    public function parseCatalog(string $html, string $url): array
    {
        ...
        $products = $this->extractProductsFromJsonLd($document, $url);
        if ($products !== []) {
            return $products;
        }
        ...
        if ($products === []) {
            throw new ProductParseException(
                "Не удалось извлечь товары из каталога Перекрёстка.",
            );
        }
     ...
     */
    private function extractProductsFromJsonLd(Document $document, string $catalogUrl): array
    {
        $products = [];
        $seenUrls = [];
        ...
        foreach ($document->find('script[type="application/ld+json"]') as $script) {
            ...
            foreach ($this->flattenNodes($decoded) as $node) {
                ...
                $productUrl = $this->makeAbsoluteUrl($rawUrl, $catalogUrl);
                $path = (string) parse_url($productUrl, PHP_URL_PATH);

                if (preg_match('~/cat/(?:c/)?\d+/p/~', $path) !== 1) {
                    continue;
                }
    ...
    private function extractProductUrl(Element $link, string $catalogUrl): ?string
    {
        ...
        return preg_match('~/cat/(?:c/)?\d+/p/~', $path) === 1 ? $absoluteUrl : null;
    }
    ...
    private function makeAbsoluteUrl(string $href, string $catalogUrl): string
    {
        ...
        $origin = "https://" . ((string) parse_url($catalogUrl, PHP_URL_HOST));
    ...
    private function extractName(Element $link, string $text): ?string
    {
        ...
        $cleaned = preg_replace('/цена\s*/ui', " ", $text) ?? $text;
        $cleaned = preg_replace('/старая цена\s*/ui', " ", $cleaned) ?? $cleaned;
        $cleaned = preg_replace('/\d+(?:[.,]\d{2})?\s*₽(?:\/шт)?/u', " ", $cleaned) ?? $cleaned;
        $cleaned = preg_replace('/\b(в наличии много|в корзину|реклама|скидка)\b/ui', " ", $cleaned) ?? $cleaned;
        $cleaned = preg_replace('/\d+\s+оцен\w+.*$/ui', " ", $cleaned) ?? $cleaned;
    ...
    private function extractPriceFromText(string $text): ?float
    {
        ...
        foreach ($matches as $match) {
            $value = round((float) str_replace(",", ".", $match[1]), 2);
            if ($value > 0) {
                return $value;
            }
        }
